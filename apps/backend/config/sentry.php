...
/**
 * Sentry Laravel SDK configuration file — see docs/MONITORING.md §Error
 * reporting. Off by default: `dsn` is null unless SENTRY_LARAVEL_DSN is
 * set, and the SDK is specifically designed to be a complete no-op (no
 * network calls, no overhead beyond a config check) with no DSN — the
 * same "real vendor, off until configured" pattern this project already
 * uses for payments/SMS/push/email/maps (see docs/SECURITY.md §Secrets),
 * except this is a genuine library dependency rather than an
 * interface+fake-adapter pair, since Sentry has no meaningful "fake mode"
 * to build against and its own SDK already provides the off switch.
 * Hooks into Laravel's existing `report($e)` call automatically (see
 * App\Exceptions\ApiExceptionRenderer) — no application code changes
 * needed once a real DSN exists.
 *
