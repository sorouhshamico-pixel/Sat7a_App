...
function LoginForm() {
  ...
  async function handleSendOtp(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await postJson("/api/auth/provider/otp/send", { phone });
  ...
  async function handleVerify(event: FormEvent<HTMLFormElement>) {
    ...
    const result = await postJson("/api/auth/provider/otp/verify", { phone: step.phone, code });
    ...
    router.push(searchParams.get("next") || "/provider");
    router.refresh();
  }
  ...
  return (
    <div className="flex flex-1 items-center justify-center">
      <Card className="w-full max-w-sm">
        <CardTitle>تسجيل دخول مزودي الخدمة</CardTitle>
